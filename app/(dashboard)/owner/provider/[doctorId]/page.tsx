...
export default function DoctorDetailsPage({
  ...
  const loadDoctor = async () => {
    const { data } = await supabase
      .from("doctors")
      .select(
        "*, profiles(full_name, email), departments(name), specializations(name), businesses(business_type)"
      )
    ...
    return (
      <div className="p-6">
        <EmptyState
          ...
          description="The provider record could not be loaded."
        />
      </div>
    );
  }
  ...
  return (
    <div className="p-6 space-y-6">
      <PageHeader
        title={doctor.profiles?.full_name ?? providerLabel}
        description={doctor.specializations?.name ?? `${providerLabel} profile`}
      />

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>{providerLabel} Details</CardTitle>
          </CardHeader>
